...
**But nobody can currently say whether the yield justifies the cost, and the reason is structural.** The [capsule cost ledger](https://github.com/mbackschat/bpmn-lean-experiment/blob/0adda45/docs/CAPSULE-COST-LEDGER.md) measures nonblank code and documentation churn — explicitly *not* *"proof strength, test independence, JSON evidence volume, generated output, or wall time."* Review effort is none of the things it counts. Elapsed time is `Unknown` on every row for want of reliable timestamps, which is the correct refusal and leaves the question unanswerable from the instrument.
