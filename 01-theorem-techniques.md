...
And the left-hand box is only half true. The [profile-parameterized admission work](https://github.com/mbackschat/bpmn-lean-experiment/blob/0adda45/docs/PROFILE-PARAMETERIZED-ADMISSION-SPEC.md) split admission into a reusable topology-independent graph/program validator plus a per-profile mechanism-and-cardinality capability, and an executable architecture guard *prohibits* reintroducing a whole-topology execution-surface predicate. So the graph-shape half of admission is generalised; the capability half is still an enumerated multiset per profile.
