...
#
# The baseline commit is parsed from README.md's version box, so there is one
# source of truth for it. Override the repository location with
# BPMN_PROJECT_ROOT when it is not at the default sibling path.
#
...
baseline=$(sed -n 's/.*\*\*Measurement baseline\*\* | `\([0-9a-f]\{7,40\}\)`.*/\1/p' README.md | head -1)

if [ -z "$baseline" ]; then
  echo "check-staleness: FAIL — no measurement baseline found in README.md"
  echo "  expected a version-box row: | **Measurement baseline** | \`<sha>\` … |"
  exit 1
fi
...
printf '%s\n' \
  ...
  "packages/temporal-adapter/src|07 08 — adapter size and hosting" \
